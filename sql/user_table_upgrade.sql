...
-- =============================================
-- 生产批次四表（produce_batch / produce_pallet / produce_goods / produce_batch_destination）
-- 先删后建，主键由MyBatis-Plus雪花算法生成，不使用数据库自增
-- 无外键约束、无唯一约束
-- =============================================

-- 先删子表再删父表（按依赖顺序）
IF OBJECT_ID('dbo.produce_goods', 'U') IS NOT NULL DROP TABLE dbo.produce_goods;
IF OBJECT_ID('dbo.produce_batch_destination', 'U') IS NOT NULL DROP TABLE dbo.produce_batch_destination;
IF OBJECT_ID('dbo.produce_pallet', 'U') IS NOT NULL DROP TABLE dbo.produce_pallet;
IF OBJECT_ID('dbo.produce_batch', 'U') IS NOT NULL DROP TABLE dbo.produce_batch;
...
CREATE TABLE dbo.produce_pallet (
    ...
    virtual_id              NVARCHAR(64)   NULL,  -- 虚拟托盘ID
    load_status             CHAR(1)        NOT NULL DEFAULT '0',  -- 上货状态 0-未上货 1-已上货
    load_time               DATETIME2      NULL,
    send_destination_code   NVARCHAR(32)   NULL,  -- 发送的目的地编码
    send_status             CHAR(1)        NOT NULL DEFAULT '0',  -- 是否已发送目的地 0-未发送 1-已发送
    send_time               DATETIME2      NULL,
    invalid_flag            CHAR(1)        NOT NULL DEFAULT '0',
    created_at              DATETIME2      NOT NULL DEFAULT SYSDATETIME()
);
